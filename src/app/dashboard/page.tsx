...
export default function Dashboard() {
  ...
  if (!institutionId) {
    return (
      <div className="p-4 md:p-12 text-center space-y-6">
        <div className="size-16 md:size-20 bg-muted rounded-full flex items-center justify-center mx-auto">
          <Activity className="size-8 md:size-10 text-muted-foreground/30" />
        </div>
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-300 pb-20">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button 
            variant="outline" 
            className="flex-1 sm:flex-none gap-2 h-11 px-4 md:px-6 rounded-xl transition-all active:scale-95" 
            onClick={handleGenerateVideo} 
            disabled={videoLoading}
          >
            {videoLoading ? <Loader2 className="size-4 animate-spin" /> : <PlayCircle className="size-4" />}
            <span className="truncate">AI Virtual Tour</span>
          </Button>
          <Button className="flex-1 sm:flex-none bg-primary h-11 px-4 md:px-8 rounded-xl shadow-lg shadow-primary/10 transition-all active:scale-95" asChild>
        ...
        {[
          ...
          <Card key={stat.title} className="overflow-hidden border-none shadow-md bg-white hover:shadow-xl transition-all duration-300">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <div className={`size-8 rounded-lg ${stat.bg} flex items-center justify-center shrink-0`}>
                <stat.icon className={`size-4 ${stat.color}`} />
              </div>
            </CardHeader>
            <CardContent>
              <div className="text-2xl md:text-3xl font-bold font-headline truncate">{stat.value}</div>
              <p className="text-[10px] text-muted-foreground mt-1 font-bold uppercase tracking-tighter truncate">{stat.label}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="grid gap-6 md:gap-8 lg:grid-cols-3">
        <Card className="lg:col-span-2 border-none shadow-lg bg-white overflow-hidden rounded-2xl md:rounded-3xl">
          <CardHeader className="border-b bg-slate-50/50 p-4 md:p-6">
            <div className="flex justify-between items-center">
              <div>
                <CardTitle className="text-lg md:text-xl font-headline font-bold">Academic Lifecycle</CardTitle>
                <CardDescription className="text-xs md:text-sm">Real-time synchronization across modules.</CardDescription>
              </div>
            ...
            <div className="space-y-8">
              <div className="space-y-3">
                <div className="flex justify-between text-xs md:text-sm">
                  <span className="font-bold flex items-center gap-2"><CheckCircle2 className="size-4 text-green-600 shrink-0" /> Attendance Consistency</span>
                  <span className="text-muted-foreground">{attendanceProgress}%</span>
                </div>
                <Progress value={attendanceProgress} className="h-2 rounded-full" />
              </div>
              <div className="space-y-3">
                <div className="flex justify-between text-xs md:text-sm">
                  <span className="font-bold flex items-center gap-2"><FileText className="size-4 text-blue-600 shrink-0" /> Continuous Assessment</span>
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-xs md:text-sm">
                  <span className="font-bold flex items-center gap-2"><Wallet className="size-4 text-amber-600 shrink-0" /> Fee Collection Cycle</span>
          ...
          <Card className="border-none shadow-md bg-primary text-primary-foreground rounded-2xl md:rounded-3xl overflow-hidden group">
            <CardHeader className="p-4 md:p-6">
              <CardTitle className="text-base md:text-lg flex items-center gap-2">
                <ShieldCheck className="size-5 shrink-0" /> Institutional Safety
              </CardTitle>
              <CardDescription className="text-xs text-primary-foreground/60">Global audit active for data.</CardDescription>
            </CardHeader>
            <CardContent className="p-4 md:p-6 pt-0">
              <div className="p-4 rounded-xl bg-white/10 space-y-3">
                 <div className="flex items-center justify-between text-[10px] md:text-xs"><span>Data Isolation</span><Badge className="bg-green-500/20 text-green-400 border-none h-4 px-1.5">Verified</Badge></div>
                 <div className="flex items-center justify-between text-[10px] md:text-xs"><span>Role Access</span><Badge className="bg-blue-500/20 text-blue-400 border-none h-4 px-1.5">Active</Badge></div>
                 <div className="flex items-center justify-between text-[10px] md:text-xs"><span>Audit Integrity</span><Badge className="bg-amber-500/20 text-amber-400 border-none h-4 px-1.5">Synced</Badge></div>
              </div>
              <Button variant="secondary" className="w-full mt-4 md:mt-6 h-11 rounded-xl font-bold bg-white text-primary group-hover:scale-[1.01] transition-transform active:scale-95" asChild>
                <Link href="/dashboard/logs">View Security Trail</Link>
              </Button>
            </CardContent>
          </Card>

          <Card className="border-none shadow-md bg-white rounded-2xl md:rounded-3xl overflow-hidden">
            <CardHeader className="pb-2 p-4 md:p-6">
               <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Logistics Hub</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 p-4 md:p-6 pt-0">
               <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors border cursor-pointer">
                  <div className="size-10 rounded-xl bg-orange-50 flex items-center justify-center shrink-0"><Bus className="size-5 text-orange-600" /></div>
                  <div className="min-w-0">
                    <p className="text-sm font-bold truncate">Transport Fleet</p>
                    <p className="text-[10px] text-muted-foreground uppercase truncate">Registry Operational</p>
                  </div>
               </div>
               <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors border cursor-pointer">
                  <div className="size-10 rounded-xl bg-blue-50 flex items-center justify-center shrink-0"><Package className="size-5 text-blue-600" /></div>
                  <div className="min-w-0">
                    <p className="text-sm font-bold truncate">Asset Inventory</p>
                    <p className="text-[10px] text-muted-foreground uppercase truncate">Registry Active</p>
                  </div>
               </div>
            </CardContent>
          </Card>
