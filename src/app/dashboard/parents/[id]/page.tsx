    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <Card className="border-none shadow-2xl rounded-3xl overflow-hidden bg-white">
        ...
        <Tabs defaultValue="overview" className="w-full">
           <TabsList className="bg-muted/30 px-8 py-2 border-b justify-start gap-4">
              <TabsTrigger value="overview">Identity & Details</TabsTrigger>
              <TabsTrigger value="children" className="gap-2">Children <Badge className="bg-primary text-white h-4 w-4 p-0 flex items-center justify-center text-[8px] border-none">{rels.length}</Badge></TabsTrigger>
              <TabsTrigger value="professional">Employment</TabsTrigger>
              ...
              <TabsContent value="overview" className="mt-0 space-y-8 animate-in fade-in slide-in-from-bottom-2">
                 <div className="grid gap-8 md:grid-cols-2">
                    <section className="space-y-4">
                       <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground border-b pb-2">Personal Identification</h4>
                       <div className="space-y-3">
                          <p className="flex justify-between text-sm"><span>Legal Name</span><span className="font-bold">{parent.firstName} {parent.middleName} {parent.lastName}</span></p>
                    ...
                    <section className="space-y-4">
                       ...
                       <div className="space-y-3">
                          <p className="flex justify-between text-sm"><span>Digital Address</span><span className="font-mono font-bold text-accent">{parent.digitalAddress || "Unlisted"}</span></p>
                          <p className="flex justify-between text-sm"><span>Residential</span><span className="font-bold">{parent.address}</span></p>
                          <p className="flex justify-between text-sm"><span>Town/District</span><span className="font-bold">{parent.town}, {parent.district}</span></p>
                          <p className="flex justify-between text-sm"><span>Region</span><span className="font-bold">{parent.region}</span></p>
                       </div>
                    </section>
                 </div>
              </TabsContent>
                         ...
                         <Card key={r.id} className="border-none shadow-sm bg-slate-50/50 hover:bg-slate-50 transition-colors group">
                            <CardContent className="p-6 flex items-center justify-between">
                               ...
                               <div className="flex gap-2">
                                  ...
                                  <Button variant="ghost" size="icon" asChild className="h-8 w-8"><Link href={`/dashboard/students?id=${r.studentId}`}><ArrowLeft className="size-3 rotate-180" /></Link></Button>
                               </div>
                            </CardContent>
                         </Card>
                       )
                 ...
                 <div className="p-6 rounded-2xl border bg-slate-50 space-y-6">
                    ...
                    <div className="grid gap-8 md:grid-cols-2">
                       <div className="space-y-1">
                          ...
                          <p className="font-bold text-sm">{parent.occupation || "Not Specified"}</p>
                       </div>
                       <div className="space-y-1">
                          <p className="text-[10px] font-bold uppercase text-muted-foreground">Employer / Agency</p>
                          <p className="font-bold text-sm">{parent.employer || "Self-Employed"}</p>
                       </div>
                       <div className="space-y-1 md:col-span-2">
                          <p className="text-[10px] font-bold uppercase text-muted-foreground">Office Address</p>
                          <p className="font-bold text-sm italic">"{parent.officeAddress || "No office address registered."}"</p>
                       </div>
                    </div>
                 </div>
              </TabsContent>

              <TabsContent value="documents" className="mt-0 space-y-6 animate-in fade-in slide-in-from-bottom-2">
                 <div className="grid gap-6 md:grid-cols-2">
                    <div className="p-6 rounded-2xl border bg-slate-50 space-y-3">
                       ...
                       <p className="font-mono font-bold text-primary text-base">{parent.nationalId || "Not Registered"}</p>
                    </div>
                    <div className="p-6 rounded-2xl border bg-slate-50 space-y-3">
                       <p className="text-[10px] font-bold uppercase text-muted-foreground">Passport Number</p>
                       <p className="font-mono font-bold text-primary text-base">{parent.passportNumber || "Not Registered"}</p>
                    </div>
                 </div>
              </TabsContent>
           </CardContent>
